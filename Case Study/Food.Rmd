...
We surveyed 126 college students about their food choices and preferences. While looking at this data a question arose. Does the number of times a college student eats out come from the same GPA distribution? Or is there evidence that at least one group of number of times eaten out comes from a different distribution of GPA than the others?

```{r}
GPA <- food %>% select(GPA, eating_out)
datatable(GPA, options=list(lengthMenu = c(5,10,25,50)), extensions="Responsive")
```
### Hypothesis
Below are the following hypothesis that we are testing.

$$
H_0: \text{All samples are from the same population.}
$$

$$
H_a: \text{At least one sample's population is stochastically different}
$$
$$
\alpha = 0.05
$$
...
## Analysis
As we look at the Q-Q plot below, we can see that the data is not normally distributed. Therefore we will be running a Kruskal-Wallis Test.

```{r,warning=FALSE, message=FALSE}
gpa.aov <- aov(as.numeric(GPA) ~ eating_out, data = GPA)
plot(gpa.aov, which =2, pch=16)
```


Here below is the Kruskal-Wallis rank sum test.

```{r, warning=FALSE, message=FALSE}
kruskal.test(as.numeric(GPA) ~ eating_out, data = food) %>%
pander()

```

It appears that all of the groups of eating out and the GPA come from the same population ($p = \text{0.498}$). Below is a side-by-side boxplot that shows the data.

```{r, warning = FALSE, message = FALSE}
boxplot(as.numeric(GPA) ~ eating_out, data=food,col="blue", ylab = "GPA", xlab = "Number of Times Eating Out in a Week", main = "The Number of Times a Student Eats Out Compared to GPA")
```

```{r, warning= FALSE, message = FALSE}
pander(favstats(as.numeric(GPA) ~ eating_out, data=food)[,-10])
```



From the data table and box plots we can see that the means are all the means from each group appears to be very similar. We can also see that the group of students that say they eat 1-2 times, 2-3 times, and Never have some outliers.

## Conclusion

With the results from the Kruskal-Wallis test we can conclude that all samples are from the same distribution. We can conclude this because $p = 0.498$. This means there is not sufficent evidence to reject the null. We can also look at the data from the boxplot and see that the data all looks very similar. We can also see that the students that eat out everyday are all closer to their mean, and the students that say they never eat out are more spread out than all of the other groups.
